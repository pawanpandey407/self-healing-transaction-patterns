...
- **Description:** The application layer is healthy but nothing reaches it, or it cannot reach what it depends on. The example I lived in production: an application migrated between container platforms that could not connect to the message broker cluster in the new environment, because connectivity assumptions from the old platform did not carry over. The application was healthy the whole time; the fault sat in the traffic path between it and the broker, at the hardware load balancer layer. An earlier version of this entry described that incident as two separate ones; it was one.
...
**Field note:** my closest encounter with this family came out of that container platform migration. One of the migrated apps was both a producer and a consumer on the message broker. After the move it could still connect to the topic it produced to, which lived in one broker cluster, but it could never establish a connection to the consumer cluster. The alerts fired, and working through log analytics and pod logs we confirmed the connection was simply not being established. What made it costly was the partial shape of the failure: every request path we owned looked healthy, and only the piece writing out events, data that later fed billing, was down. It took about four to five hours, with the app team connecting with SRE and then the load balancer team, before the path was worked out. Afterward we logged and documented the issue in detail, so if it happens again we can go straight back to exactly where the fault was.
